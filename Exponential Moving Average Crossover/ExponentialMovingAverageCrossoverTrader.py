class ExponentialMovingAverageCrossoverTrader:
    ...
    def find_exponential_mva(self, day):
        if day == self.mva_days: #calculate simple mva
            avg = 0.0
            for x in range(0, self.mva_days):
                avg += self.prices[day - x]
            self.exponential_mva = avg/self.mva_days
        else: #calculate exponential mva
            multiplier = float(2) / float(self.mva_days + 1)
            previous_ema = self.exponential_mva
            self.exponential_mva = (float(self.prices[day] - previous_ema) * multiplier) + previous_ema
        return self.exponential_mva
    ...
    def trade(self):
        ...
        for price in self.prices:
            if (days >= self.mva_days):
                average = self.find_exponential_mva(days)
